...
Transient Browser-control failures are explicit and sequence-safe. The server retries only connection establishment before an action begins. A `502` response carries a secret-free failure code and recovery instruction; after an ambiguous action, the copied prompt requires `observe_before_retry`, waits for `actionInProgress:false`, and compares `nextExpectedSequence` before deciding whether an action may be repeated. Cleanup-only CDP detach failure cannot replace an otherwise successful command response.
...
The reset command requires the exact Redis scope, advances a server-side quota epoch, and records the previous/current epoch plus reset time. It resets the current daily counters without deleting active leases or releasing browsers, and it exposes no control in the public UI. With trusted local Upstash credentials it writes Redis directly. Otherwise it calls the existing cleanup function with an exact confirmation contract and a constant-time-checked `SOLARI_REMOTE_OWNER_TOKEN`. While `SOLARI_EVALUATION_ENABLED=false`, it may fall back to the existing private `SOLARI_EVALUATION_TOKEN`; that fallback fails closed as soon as live evaluation is enabled, at which point a separate owner token is mandatory. Redis credentials therefore remain server-side even though Vercel redacts Sensitive Marketplace values. Keep the owner token only in trusted local/Production environments; never prefix it with `VITE_`, put it in the copied agent prompt, or weaken a Production secret to make it pullable.
