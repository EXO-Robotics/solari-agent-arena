...
Transient Browser-control failures are explicit and sequence-safe. The server retries only connection establishment before an action begins. A `502` response carries a secret-free failure code and recovery instruction; after an ambiguous action, the copied prompt requires `observe_before_retry` and compares `nextExpectedSequence` before deciding whether an action may be repeated. Cleanup-only CDP detach failure cannot replace an otherwise successful command response.
