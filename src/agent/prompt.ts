...
An HTTP 502 with retryable=true is a temporary Browser-control failure, not proof that an Act did or did not execute. The server marks ambiguous Act failures with recovery=observe_before_retry. Never retry an Act blindly. After an Act returns 502, call Observe with 1s, 2s, then 4s backoff, stopping only when one succeeds with actionInProgress=false. Compare its nextExpectedSequence with the failed Act's expectedSequence: if it advanced by one, the Act was accepted and settled; if it is unchanged, retry that Act once; if it is anything else, disconnect and report the mismatch. If Observe succeeds with actionInProgress=true, keep waiting within the same bounded recovery sequence and do not act. For a 502 from Observe, retry Observe with the same bounded backoff. For HTTP 409 Expected action sequence N or code=action_in_progress, Observe until actionInProgress=false and continue from N. If three recovery observations fail or remain busy, disconnect safely and report the last confirmed observation.
